...
void main() async {
  ...
  // إعداد الإشعارات مع الصوت الافتراضي القوي (ستسمع رنة تنبيه قوية عند الحجز)
  const AndroidInitializationSettings initializationSettingsAndroid = AndroidInitializationSettings('@mipmap/ic_launcher');
  const InitializationSettings initializationSettings = InitializationSettings(android: initializationSettingsAndroid);
  await flutterLocalNotificationsPlugin.initialize(initializationSettings);

  // إنشاء قناة الإشعارات بصوت قوي واهتزاز
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      if (notification != null && android != null) {
        flutterLocalNotificationsPlugin.show(
          ...
          const NotificationDetails(android: AndroidNotificationDetails('high_importance_channel', 'إشعارات الحجوزات', importance: Importance.max, priority: Priority.high, playSound: true, enableVibration: true, icon: '@mipmap/ic_launcher')),
        );
      }
    });
  }
