...
// --- الثيم العام للتطبيق (Dark & Light Mode) ---
...
void main() async {
  ...
  // طلب صلاحيات الإشعارات وتفعيلها
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          Image.network(
            'https://images.unsplash.com/photo-1622286342621-4bd786c2447c?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80',
            fit: BoxFit.cover,
          ),
        ...
        if (snapshot.hasData) {
          return FutureBuilder<DocumentSnapshot>(
            ...
            builder: (context, userSnap) {
              ...
              // حفظ Token الإشعارات عند الدخول
              FirebaseMessaging.instance.getToken().then((token) {
                if (token != null) {
                  FirebaseFirestore.instance.collection('users').doc(snapshot.data!.uid).update({'fcmToken': token});
                }
              });
  ...
  void _showAvatarDialog(BuildContext context, String currentUrl) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(onPressed: () => Navigator.pop(context), child: Text(isArabic ? 'إلغاء' : 'Annuler', style: TextStyle(color: Theme.of(context).textTheme.bodyMedium!.color))),
          ElevatedButton(
            ...
            child: Text(isArabic ? 'حفظ' : 'Enregistrer', style: const TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
          ...
          child: Column(
            children: [
              ...
              ListTile(
                ...
                title: Text(isArabic ? 'لغة التطبيق (Français)' : 'Langue (العربية)'),
                onTap: onLanguageToggle,
              ),
              ListTile(
                leading: const Icon(Icons.brightness_6, color: Color(0xFFD4AF37)),
                title: Text(isArabic ? 'تغيير المظهر (ليلي/نهاري)' : 'Changer le thème'),
                onTap: () {
              ...
              ListTile(
                ...
                title: Text(isArabic ? 'تسجيل الخروج' : 'Déconnexion', style: const TextStyle(color: Colors.redAccent)),
...
class _AuthScreenState extends State<AuthScreen> {
    ...
    try {
      if (isLogin) {
        ...
        await FirebaseFirestore.instance.collection('users').doc(c.user!.uid).set({
          ...
          'totalBookings': 0,
          'available': true,
          'favorites': [], // قائمة المفضلة للزبون
          'gallery': [], // معرض الصور للحلاق
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: Stack(
        children: [
          Container(decoration: const BoxDecoration(image: DecorationImage(image: NetworkImage('https://images.unsplash.com/photo-1585747860715-2ba37e788b70?ixlib=rb-4.0.3&auto=format&fit=crop&w=1000&q=80'), fit: BoxFit.cover))),
...
// -------------------------------------------------------------
// واجهة الزبون (Customer Dashboard)
// -------------------------------------------------------------
...
class _CustomerDashboardState extends State<CustomerDashboard> {
  ...
  int _tab = 0; // 0 for Home, 1 for Favorites

  // دالة لفتح الواتساب أو الهاتف
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  showDialog(context: context, builder: (_) => AlertDialog(
                    title: Text(widget.isArabic ? 'إضافة صورة (رابط)' : 'Ajouter Image URL'),
                    content: TextField(controller: ctrl, decoration: const InputDecoration(hintText: 'https://...')),
                    actions: [
                      TextButton(onPressed: () {
                        if(ctrl.text.isNotEmpty) {
                          gallery.add(ctrl.text);
                          FirebaseFirestore.instance.collection('users').doc(widget.uid).update({'gallery': gallery});
                        }
                        Navigator.pop(context);
                      }, child: const Text('حفظ', style: TextStyle(color: Color(0xFFD4AF37))))
                    ],
                  ));
                },
              )
